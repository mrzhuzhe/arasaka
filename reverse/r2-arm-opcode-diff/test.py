# notice in this case Carry flag is fixed to 0
...
for line in disassembly:
        ...
        if "mov" in line:
            ...
            print("action", line, hex(R0 & 0xFFFFFFFF), hex(R1 & 0xFFFFFFFF), hex(R2 & 0xFFFFFFFF))
            used=1
        if any(op in line for op in ["r0, r0, #0", "r0, r0, r1"]):
            ...
            if action == "add":
                ...
                R0 = (R0 - R1 - R2) + 0xFFFFFFFF # Warnning this is different to radare2 
            elif action == "and":
                R0 &= R1 & R2
            elif action == "eor":
                R0 ^= R1 ^ R2
